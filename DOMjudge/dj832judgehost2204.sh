...
#DOMjudge8.3.2 stable(2025.07.09) + Ubuntu 22.04 LTS + apache2/nginx
...
#wget https://raw.githubusercontent.com/melongist/CSL/master/DOMjudge/dj832judgehost.sh
#bash dj832judgehost.sh
...
wget https://raw.githubusercontent.com/melongist/CSL/master/DOMjudge/dj832start.sh
sudo sed -i "s/\$USER/${USER}/g" ~/dj832start.sh

#Number of judgehosts autoscaling for the number of cpu change to /etc/rc.local
echo '#!/bin/bash' >> ~/rc.local
echo "bash /home/${USER}/dj832start.sh" >> ~/rc.local
...
#echo "------" | tee -a ~/${README}
#echo "bash dj832start.sh" | tee -a ~/${README}
#echo "" | tee -a ~/${README}
#echo "" | tee -a ~/${README}
