...
if (isset($_GET['sid']) && isset($_SESSION[$OJ_NAME.'_'.'administrator']) && $_GET['command'] == "rejudge") {
        $sid = intval($_GET['sid']);
        $sql="UPDATE `solution` SET `result`=1 WHERE `solution_id`=?";
        pdo_query($sql, $sid);
        $url="status.php?top=".($sid+1);
...
for ($i=0; $i<$rows_cnt; $i++) {
  ...
  if(isset($_SESSION[$OJ_NAME.'_'.'administrator']))
  {
    $view_status[$i][0] .= "<a href='status.php?command=rejudge&sid=".$row['solution_id']."'> <i class='glyphicon glyphicon-repeat'></i></a>";
    $view_status[$i][0] .= "<a class='text-danger' id='del-".$row['solution_id']."' href='#'> <i class='glyphicon glyphicon-trash'></i></a>";
  }
  // + by CSL from CodeUp
