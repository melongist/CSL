...
for ($i=0; $i<$rows_cnt; $i++) {
  ...
  if ((intval($tresult)!=0) && (!isset($_SESSION[$OJ_NAME.'_'.'source_browser'])))    //if the problem will be use remaind contes/exam
    $hide = true;  
  ...
  if ($row['contest_id']>0) {
    if (isset($end_time) && time() < $end_time) {
      ...
      if (isset($cid)) {
        if ($hide == true)   //if the problem will be use remaind contes/exam
